                    ...
                    <div className={styles.bodyRight}>
                        <div className={styles.boxContainer}>
                            ...
                            <div className={styles.infoContainer}>
                                <div>
                                    <span>{ls.LS_PROBLEM_DETAIL_PROBLEM_TYPE_LABEL}</span>{" "}
                                    {problemTypeText(problemDetail.type)}
                                </div>
                                {problemDetail.judgeInfo && (
                                    <>
                                        {problemDetail.type !== CE_ProblemType.SubmitAnswer && (
                                            <>
                                                <div>
                                                    <span>{ls.LS_PROBLEM_DETAIL_TIME_LIMIT_LABEL}</span>{" "}
                                                    {problemDetail.judgeInfo.timeLimit}ms
                                                </div>
                                                <div>
                                                    <span>{ls.LS_PROBLEM_DETAIL_MEMORY_LIMIT_LABEL}</span>{" "}
                                                    {problemDetail.judgeInfo.memoryLimit}
                                                    MiB
                                                </div>
                                                <div>
                                                    <span>{ls.LS_PROBLEM_DETAIL_IO_LABEL}</span>{" "}
                                                    {problemDetail.judgeInfo.fileIO &&
                                                    problemDetail.type !== CE_ProblemType.Interaction
                                        ...
                                        {problemDetail.type === CE_ProblemType.Traditional &&
                                            problemDetail.judgeInfo.fileIO && (
                                                <>
                                                    <div>
                                                        <span>{ls.LS_PROBLEM_DETAIL_INPUT_FILE_LABEL}</span>{" "}
                                                        {problemDetail.judgeInfo.inputFile}
                                                    </div>
                                                    <div>
                                                        <span>{ls.LS_PROBLEM_DETAIL_OUTPUT_FILE_LABEL}</span>{" "}
                                                        {problemDetail.judgeInfo.outputFile}
                                                    </div>
                                                </>
                                            )}
                                    </>
                                    ...
                                    <div>
                                        ...
                                        <FluentRouterLink
                                            to={makeUrl({
                                                ...
                                                params: { id: problemDetail.owner.id },
                                            })}
                                        >
                                ...
                                {problemDetail.isPublic && problemDetail.publicTime && (
                                    <div>
                                        <span>{ls.LS_PROBLEM_PUBLIC_TIME_LABEL}</span>{" "}
                                        {momentFormatter(problemDetail.publicTime, "lll")}
                                    </div>
                                )}
                            </div>
                        </div>
